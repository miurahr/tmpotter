...
public final class Segmenter {
   ...
   * <p>
   * As sentences are returned by {@link #segment(String, List)} without spaces
          ...
          if (matcher.find()) {
            ...
            if (!leftSpaces.isEmpty()) {
              sp.replace(0, leftSpaces.length(), "");
                  ...
                  && (!AppConstants.SPACY_REGEX
                  .matcher(rule.getBeforebreak()).matches()
                  || !AppConstants.SPACY_REGEX.
   ...
   */
  public static void segmentEntries(boolean needResegment,
          Language sourceLang, String sourceEntry,
          Language targetLang, String targetEntry,
          List<String> sourceSegments, List<String> targetSegments) {
    if (needResegment) {
      List<String> srcSegments = Segmenter.segment(sourceLang,
              sourceEntry, null, null);
            // There is no translation for this entry, because for instance
      // it's a note on an untranslated entry
      if (targetEntry != null) {
        List<String> tarSegments = Segmenter.segment(targetLang,
                targetEntry, null, null);
